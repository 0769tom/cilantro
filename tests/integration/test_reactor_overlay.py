...
class TestReactorOverlay(MPTestCase):
    ...
    def test_pubsub_1_pub_2_sub_unauth(self):
        ...
        def assert_sub(test_obj):
            ...
            log.important("type of testobj.handle_pub: {}".format(type(test_obj.handle_pub)))
        ...
        time.sleep(5)  # nap while overlay hooks up

        pub.create_pub_unauth(ip=pub.ip)
        sub.create_sub_unauth(vk=PUB1_VK)

        time.sleep(5)  # for the vk lookup?
